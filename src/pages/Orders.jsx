...
const Orders = () => {
  const { isVendor, orders, simulateLoading } = useContext(AppContext);
        ...
        {loading ? (
          ...
          <div className="space-y-6" role="list">
            {currentOrders.map(order => {
              ...
              return (
                <div key={order.id} className="bg-black/10 rounded-2xl p-6 transition-all duration-300" role="listitem" aria-label={`Order ${order.id}, total ₹${order.total.toFixed(2)}, status ${order.status}`}>
                  <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div className="flex-1">
